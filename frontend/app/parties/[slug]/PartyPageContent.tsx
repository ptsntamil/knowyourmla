...
interface PartyPageContentProps {
  slug: string;
  election: string;
}

export default async function PartyPageContent({ slug, election }: PartyPageContentProps) {
  ...
  return (
    <div className="min-h-screen bg-page-bg selection:bg-brand-gold/20">
      ...
      <React.Suspense fallback={<div className="h-12 w-full bg-white animate-pulse" />}>
        <ElectionFilter 
          options={availableElections || []} 
          basePath={`/parties/${slug}`}
          currentElection={election || "all"}
        />
      </React.Suspense>
