  ...
  return (
    <div className="min-h-screen bg-bg-page">
      ...
      <main className="max-w-7xl mx-auto px-4 py-10 space-y-12">
        <nav className="flex items-center flex-wrap gap-y-2 text-xs md:text-[13px] font-black uppercase tracking-[0.2em] text-text-muted">
          {breadcrumbItems.map((item, index) => (
            <div key={`${item.item}-${index}`} className="flex items-center">
              {index > 0 && <span className="mx-2 text-border-subtle">/</span>}
              {index === breadcrumbItems.length - 1 ? (
                <span className="text-text-primary font-black">{item.name}</span>
              ) : (
                <a href={item.item} className="hover:text-text-accent transition-all active:scale-95 px-2 py-1 -ml-2 rounded outline-none focus-visible:ring-2 focus-visible:ring-text-accent">
                  {item.name}
                </a>
              )}
            </div>
        ...
        <section className="bg-bg-page rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden border border-border-subtle">
          <div className="absolute top-0 right-0 w-96 h-96 bg-bg-surface rounded-full -mr-48 -mt-48 blur-3xl pointer-events-none" />
 
           <div className="flex justify-between items-center mb-12 relative z-10">
            <h2 className="text-3xl font-black text-text-primary uppercase tracking-tighter">Performance Metrics</h2>
            <span className="text-[10px] bg-bg-accent text-text-inverse font-black px-4 py-1.5 rounded-full uppercase tracking-[0.2em] border border-border-subtle shadow-lg">Win Strength</span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
            <div className="bg-bg-card rounded-3xl p-8 shadow-2xl flex flex-col h-full border border-border-subtle">
              <h3 className="text-xl font-black text-text-primary mb-8 uppercase tracking-tight">Asset Growth</h3>
              <div className="flex-1">
                <AssetChart data={profile.analytics.asset_growth.map((a) => ({ year: a.year, assets: a.assets }))} />
              </div>
              <p className="text-[11px] text-text-muted font-bold mt-8 leading-relaxed opacity-70">
                Visualize the growth of assets declared by {profile.person.name} across different election cycles.
              </p>
            </div>

            <div className="bg-bg-card rounded-3xl p-8 shadow-2xl flex flex-col h-full border border-border-subtle">
              <h3 className="text-xl font-black text-text-primary mb-8 uppercase tracking-tight">Vote Share</h3>
              <div className="flex-1">
            ...
            <div className="bg-bg-card rounded-3xl p-8 shadow-2xl flex flex-col h-full border border-border-subtle">
              <h3 className="text-xl font-black text-text-primary mb-8 uppercase tracking-tight">Margin Trend</h3>
              <div className="flex-1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 space-y-12">
            <section className="space-y-8">
              <h2 className="text-3xl font-black text-text-primary uppercase tracking-tighter">Election Participation</h2>
              <div className="bg-bg-card rounded-[2rem] shadow-xl border border-border-subtle overflow-hidden">
                <div className="px-10 py-8 border-b border-border-subtle flex justify-between items-center bg-bg-surface">
                  <h3 className="text-xl font-black text-text-primary uppercase tracking-wider">Historical Records</h3>
                  <div className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em]">
                    Win Rate: <span className="text-text-accent">{profile.analytics.win_rate.win_rate}%</span>
                  </div>
                </div>
            ...
            <section className="space-y-8">
              <h2 className="text-3xl font-black text-text-primary uppercase tracking-tighter">Income Details</h2>
              <IncomeDetailsTable itrHistory={profile.analytics.itr_history} />
            </section>

            <section className="space-y-8">
              <h2 className="text-3xl font-black text-text-primary uppercase tracking-tighter">Impact Widgets</h2>
          ...
          <aside className="space-y-12">
            ...
            <section className="space-y-8">
              <h2 className="text-3xl font-black text-text-primary uppercase tracking-tighter">Insights</h2>
              <div className="space-y-8">
                <div className="bg-bg-surface p-10 rounded-[2rem] shadow-2xl border border-border-subtle relative overflow-hidden group hover:scale-[1.02] transition-transform">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-text-accent/10 rounded-full -mr-16 -mt-16 blur-xl pointer-events-none" />
                  <h3 className="text-text-accent font-black uppercase tracking-[0.3em] text-[10px] mb-6">Did you know?</h3>
                  <div className="space-y-6">
                    <p className="text-text-primary text-base leading-relaxed font-bold tracking-tight">
                      This {personalTitle === "MLA" ? "MLA" : "candidate"} has contested in <span className="text-text-accent underline decoration-2 underline-offset-4 font-black">{profile.analytics.win_rate.total_contested}</span> elections and won <span className="text-text-accent underline decoration-2 underline-offset-4 font-black">{profile.analytics.win_rate.total_wins}</span> of them.
                    </p>
                    <p className="text-text-muted text-sm leading-relaxed font-bold tracking-tight opacity-90">
                      ...
                      <a
                        ...
                        className="text-text-accent hover:underline decoration-1 underline-offset-2"
                      >
                        Source
                      </a>
                    </p>
                ...
                <div className="bg-bg-card p-10 rounded-[2rem] shadow-sm border border-border-subtle flex flex-col justify-center">
                  <h3 className="text-text-primary font-black uppercase tracking-[0.2em] text-[10px] mb-6">Internal Links</h3>
                  <ul className="space-y-6">
                    <li>
                      <a href={`/tn/constituency/${latestElection?.constituency.toLowerCase()}`} className="text-text-primary hover:text-text-accent font-black text-sm flex items-center gap-3 transition-all active:translate-x-1 py-1 group outline-none focus-visible:ring-2 focus-visible:ring-text-accent rounded px-2 -ml-2">
                        <span className="w-1.5 h-1.5 bg-text-accent rounded-full group-hover:scale-150 transition-transform" />
                        View {latestElection?.constituency} history
                      </a>
                    </li>
                    <li>
                      <a href={`/parties/${party.toLowerCase()}`} className="text-text-primary hover:text-text-accent font-black text-sm flex items-center gap-3 transition-all active:translate-x-1 py-1 group outline-none focus-visible:ring-2 focus-visible:ring-text-accent rounded px-2 -ml-2">
                        <span className="w-1.5 h-1.5 bg-text-accent rounded-full group-hover:scale-150 transition-transform" />
                        View all {party} MLAs
                      </a>
                    </li>
