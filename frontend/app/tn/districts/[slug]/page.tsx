      ...
      <main className="max-w-7xl mx-auto px-4 py-16 space-y-16">
        ...
        <div className="grid lg:grid-cols-3 gap-12 items-start">
          <div className="lg:col-span-2 space-y-12">
            <div>
              <h2 className="text-3xl font-black text-text-primary uppercase tracking-tighter mb-2">Constituencies</h2>
              <p className="text-xs font-bold text-text-muted uppercase tracking-widest">Legislative segments within {slug} district</p>
            </div>
            <ConstituencyList constituencies={constituencies} />
          </div>

          {latestStats && (latestStats.male !== undefined || latestStats.total_electors > 0) && (
            <div className="lg:col-span-1 bg-bg-card rounded-[3rem] border border-border-subtle overflow-hidden shadow-sm p-10 space-y-10 lg:sticky lg:top-8">
              <div className="space-y-2">
                <h3 className="font-black text-text-primary uppercase tracking-widest text-sm">District Electorate</h3>
                <p className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em]">Aggregated Data ({latestStats.year})</p>
              </div>
              ...
              <div className="space-y-8">
                <div className="space-y-3">
                  <div className="flex justify-between items-end">
                    ...
                    <span className="text-sm font-black text-text-primary" suppressHydrationWarning>{latestStats.male?.toLocaleString()}</span>
                  </div>
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-end">
                    ...
                    <span className="text-sm font-black text-text-primary" suppressHydrationWarning>{latestStats.female?.toLocaleString()}</span>
                  </div>
                  <div className="h-4 bg-bg-surface rounded-full overflow-hidden">
                    <div
                ...
                {latestStats.third_gender !== undefined && latestStats.third_gender > 0 && (
                  <div className="space-y-3">
                    <div className="flex justify-between items-end">
                      ...
                      <span className="text-sm font-black text-text-primary" suppressHydrationWarning>{latestStats.third_gender.toLocaleString()}</span>
                    </div>
                    <div className="h-4 bg-bg-surface rounded-full overflow-hidden">
                      <div
              ...
              <div className="pt-6 border-t border-border-subtle">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg font-black text-text-accent" suppressHydrationWarning>{latestStats.total_electors.toLocaleString()}</span>
                </div>
              </div>

              <div className="bg-bg-surface rounded-2xl p-6">
                <p className="text-[9px] font-bold text-text-muted uppercase leading-relaxed text-center">
