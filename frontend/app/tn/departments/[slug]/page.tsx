...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return buildMetadata({
    title: `${titleName} Department History | Tamil Nadu`,
    description: `Complete historical timeline of ministers who held the ${titleName} department in Tamil Nadu.`,
    path: `/tn/departments/${normalizedSlug}`,
    keywords: [`${titleName} department`, `tamil nadu departments`, `history of ${titleName} department`]
  });
}
...
export default async function DepartmentPage({ params }: PageProps) {
  ...
  const timeline = await portfolioService.getDepartmentHistory(slug);
  ...
  const breadcrumbs = [
    ...
    { name: "Departments", item: "https://www.knowyourmla.in/tn/departments" },
    { name: displayTitle, item: `https://www.knowyourmla.in/tn/departments/${slug}` }
  ];
  ...
  return (
    <div className="min-h-screen bg-slate-50 selection:bg-brand-gold/30">
      ...
      <CoverImage title={`${displayTitle} Department`}>
        <div className="flex flex-col items-center space-y-4">
          <Badge variant="brand" size="sm" dot>Department History</Badge>
          <nav className="flex justify-center items-center space-x-2 text-sm font-black uppercase tracking-widest text-slate-300">
            <Link href="/tn/departments" className="hover:text-white transition-colors">Departments</Link>
      ...
      <main className="max-w-4xl mx-auto px-4 py-16 space-y-12">
        <SEOIntro 
          h1={`History of ${displayTitle} Department`}
          intro={`Historical timeline and portfolio assignments associated with the ${displayTitle} department in the Tamil Nadu government.`}
        />
        ...
        {sortedTimeline.length === 0 ? (
          <div className="bg-white p-12 rounded-[2rem] text-center border border-slate-100 shadow-xl">
            ...
            <p className="text-slate-500 mt-2">We couldn't find any historical records for this department.</p>
          </div>
        ) : (
          <>
                    ...
                    return (
                      <div key={record.id || index} className="relative pl-8">
                        ...
                        <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100 group hover:border-brand-gold/30 transition-colors">
                          ...
                          <CabinetEventBadge event={record.cabinet_event || record.change_type} />
                        </div>
                      </div>
                    );
                  })}
