...
export default function ElectionExpensesWidget({ expenses }: ElectionExpensesWidgetProps) {
  ...
  return (
    <div className="bg-bg-card rounded-[2rem] p-8 shadow-xl border border-border-subtle mb-8 overflow-hidden relative group">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h3 className="text-sm font-black text-text-muted uppercase tracking-[0.2em] mb-1">
            Campaign Spending
          </h3>
        ...
        <div className="w-10 h-10 bg-bg-surface rounded-xl flex items-center justify-center border border-border-subtle">
      ...
      <div className="space-y-4">
        {expenses.sort((a, b) => b.year - a.year).map((record) => (
          <div key={record.year} className="p-4 rounded-2xl bg-bg-surface border border-border-subtle hover:border-text-accent/30 transition-all group-hover:bg-bg-card group-hover:shadow-sm">
            <div className="flex justify-between items-end mb-2">
              <span className="text-xs font-black text-text-muted uppercase tracking-widest">{record.year} Election</span>
              <div className="flex flex-col items-end">
                <span className="text-lg font-black text-text-primary truncate max-w-[150px]">
                  {formatCurrency(record.amount)}
                </span>
                {record.growth_percent !== null && (
                  <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${record.growth_percent >= 0 ? 'bg-bg-accent/10 text-text-accent' : 'bg-bg-muted text-text-muted'}`}>
            ...
            <div className="w-full h-1.5 bg-bg-muted rounded-full overflow-hidden">
                <div 
                    className="h-full bg-text-accent rounded-full transition-all duration-1000" 
