...
import { Award, ChevronRight, AwardIcon } from 'lucide-react';
import { CandidateInsight } from '@/lib/services/election-analytics.service';
import Badge from '@/components/ui/Badge';

interface WomenWinnersProps {
  data: CandidateInsight[];
  year: number;
}

export default function WomenWinners({ data, year }: WomenWinnersProps) {
    ...
    <div className="bg-white p-8 md:p-12 rounded-[3.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 space-y-12">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h3 className="text-2xl font-black text-brand-dark uppercase tracking-tight">Women Winners</h3>
        </div>
        <Badge variant="outline" size="sm" dot className="bg-amber-50/50 border-amber-100 text-amber-600">Top Performance</Badge>
      </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="space-y-2">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    {result.personId ? (
                      <Link 
                        href={`/tn/mla/${result.personId}`}
                        className="text-[11px] font-bold text-slate-600 hover:text-brand-gold truncate max-w-[150px] leading-none transition-colors"
                      >
                        {result.name}
                      </Link>
                    ) : (
                      <span className="text-[11px] font-bold text-slate-600 truncate max-w-[150px] leading-none">{result.name}</span>
                    )}
                    <span className="text-[11px] font-bold text-slate-400 leading-none">({result.partyShort})</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-4 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6">
                <div className="text-right flex-shrink-0">
                  <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] mb-1">Winning Margin</p>
                  <p className="text-xl font-black text-amber-500 tabular-nums">{result.formattedValue}</p>
                </div>
                <Link
