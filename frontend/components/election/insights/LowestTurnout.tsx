...
interface LowestTurnoutProps {
  data: ConstituencyResult[];
  year: number;
  limit?: number;
}

export default function LowestTurnout({ data, year, limit = 5 }: LowestTurnoutProps) {
    ...
    <div className="bg-white p-8 md:p-12 rounded-[3.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 space-y-10">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h3 className="text-2xl font-black text-brand-dark uppercase tracking-tight">Lowest Turnout</h3>
        </div>
        <Badge variant="outline" size="sm" dot className="bg-rose-50/50 border-rose-100 text-rose-600">Low Participation</Badge>
      </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-4 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6">
                <div className="text-right flex-shrink-0">
                  ...
                  <p className="text-xl font-black text-rose-500 tabular-nums">{result.turnoutPercent?.toFixed(2)}%</p>
                </div>
                <Link
                  href={`/tn/constituency/${result.constituencyId?.toLowerCase()}/election/${year}`}
