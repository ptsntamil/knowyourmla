...
interface BiggestVictoriesProps {
  data: ConstituencyResult[];
  year: number;
  limit?: number;
}

export default function BiggestVictories({ data, year, limit = 5 }: BiggestVictoriesProps) {
    ...
    <div className="bg-white p-8 md:p-12 rounded-[3.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 space-y-10">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h3 className="text-2xl font-black text-brand-dark uppercase tracking-tight">Biggest Victories</h3>
        </div>
        <Badge variant="outline" size="sm" dot className="bg-brand-gold/10 border-brand-gold/20 text-brand-gold">Landslide</Badge>
      </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-4 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6">
                <div className="text-right flex-shrink-0">
                  ...
                  <p className="text-xl font-black text-brand-green tabular-nums">+{result.margin.toLocaleString()}</p>
                </div>
                <Link
