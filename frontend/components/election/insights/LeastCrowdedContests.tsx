...
import { UserMinus, ChevronRight, Award } from 'lucide-react';
import { ConstituencyResult } from '@/lib/services/election-analytics.service';
import Badge from '@/components/ui/Badge';

interface LeastCrowdedContestsProps {
  data: ConstituencyResult[];
  year: number;
}

export default function LeastCrowdedContests({ data, year }: LeastCrowdedContestsProps) {
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 bg-slate-50 rounded-2xl flex items-center justify-center">
            <UserMinus className="text-slate-500" size={20} />
          </div>
          <h3 className="text-2xl font-black text-brand-dark uppercase tracking-tight">Least Crowded Contests</h3>
        </div>
        <Badge variant="outline" size="sm" dot className="bg-slate-50/50 border-slate-100 text-slate-600">Low Intensity</Badge>
      </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-4 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6">
                <div className="text-right flex-shrink-0">
                  <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] mb-1">Candidates</p>
                  <p className="text-2xl font-black text-slate-600 tabular-nums">{result.candidateCount}</p>
                </div>
                <Link
