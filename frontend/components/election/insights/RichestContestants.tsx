...
import { Wallet, ChevronRight, Trophy, Filter } from 'lucide-react';
import { CandidateInsight } from '@/lib/services/election-analytics.service';
import Badge from '@/components/ui/Badge';
import Image from 'next/image';

interface RichestContestantsProps {
...
export default function RichestContestants({ data, year, limit = 5, showFilter = true }: RichestContestantsProps) {
    ...
    <div className="bg-white p-8 md:p-12 rounded-[3.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 space-y-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <h3 className="text-2xl font-black text-brand-dark uppercase tracking-tight">Richest Contestants</h3>
        </div>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-4 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6">
                  <div className="text-right flex-shrink-0">
                    <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] mb-1">Assets</p>
                    <p className="text-xl font-black text-emerald-600 tabular-nums">₹{candidate.formattedValue}</p>
                  </div>
                  <Link
