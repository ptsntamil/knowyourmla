...
interface MostCrowdedContestsProps {
  data: ConstituencyResult[];
  year: number;
}

export default function MostCrowdedContests({ data, year }: MostCrowdedContestsProps) {
    ...
    <div className="bg-white p-8 md:p-12 rounded-[3.5rem] border border-slate-100 shadow-2xl shadow-slate-200/50 space-y-10">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <h3 className="text-2xl font-black text-brand-dark uppercase tracking-tight">Most Crowded Contests</h3>
        </div>
        <Badge variant="outline" size="sm" dot className="bg-indigo-50/50 border-indigo-100 text-indigo-600">High Intensity</Badge>
      </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-4 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6">
                <div className="text-right flex-shrink-0">
                  ...
                  <p className="text-2xl font-black text-indigo-600 tabular-nums">{result.candidateCount}</p>
                </div>
                <Link
