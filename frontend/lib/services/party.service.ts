...
export class PartyService {
  ...
  async getPartyBySlug(slug: string) {
    return this.partyRepo.getPartyBySlug(slug);
  }

  async getPartyElections(partyId: string) {
    const allCandidates = await this.candidateRepo.getCandidatesByParty(partyId);
  ...
  async getPartyAnalytics(partyId: string, year?: number) {
    ...
    const filteredCandidates = year 
      ? allCandidates.filter((c: any) => c.year === year)
      : allCandidates;
    ...
    if (year) {
      filteredCandidates.forEach((c: any) => {
        const hasPreviousContest = allCandidates.some((prev: any) => prev.person_id === c.person_id && prev.year < year);
  ...
  async getPartyCandidatesForYear(partyId: string, year?: number) {
    const candidates = await this.candidateRepo.getCandidatesByPartyAndYear(partyId, year);
    if (!candidates) return [];

    // Fetch history for 'is_new' badge
    const allHistory = await this.candidateRepo.getCandidatesByParty(partyId);
    ...
    return candidates.map((c: any) => {
      ...
      const effectiveYear = year || c.year;
